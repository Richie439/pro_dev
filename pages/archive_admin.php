...
$reponse = $db->prepare('SELECT * FROM user WHERE etat= 0   ORDER BY id ASC LIMIT :debut, :limite');
...
<body style="background-color:rgba(156, 156, 163, 1)">
    <div class="container bg-white pt-1 mt-5">
        <div class="container admin col-lg-12 mt-4">
            <div class="row text-white btn-lg text-center mt-5" style="background-color:RGB(51, 21, 15)">
                <span class="d-flex justify-content-center">
                    <!-- pour l'affichage sur le profil -->
                    <span class="col-1 ">
                        <!-- afficher en absence de photo par defaut abrevation du nom & prenom en  -->
                    ...
                    <span class="d-flex  mt-4  w-50" style="max-height: 2rem;">
                        ...
                        <!-- <span style="margin-left: auto">
                            <h4><a href="" class="text-white">liste des archives</a> </h4>
                        </span> -->
                    </span>

                    <div class="ml-auto  mt-3 " style="margin-left:auto;max-height: 2.5rem;">
                        <form class="d-flex" role="search">
                            <input class="form-control me-2 " name="search" action="" method="get" role="search" type="search" placeholder="Search" aria-label="Search">
                ...
                <table class="table table-striped table-bordered border border-4 border-dark">
                    ...
                    <tbody class="text-center">
                        <tr>
                            ...
                            $db = new PDO('mysql:host=127.0.0.1;dbname=Pro_dev;', 'root', '');
                            $sql = $db->query('SELECT * FROM user WHERE  etat=0');
                            $aff = false;
                            if (isset($_GET['search']) && !empty($_GET['search'])) {
                                $search = $_GET['search'];
                                $sql = $db->query("SELECT * FROM user WHERE nom LIKE '%$search%' OR prenom LIKE '%$search%' OR matricule LIKE '%$search%' AND etat=1");
                                $data = $sql->fetch();
                                $aff = true;
                            }
                            ?>
                            ...
                            if ($aff) {
                                ...
                                echo '<td class= "border border-1 border-dark d-flex justify-content-center">
    
                                    <span style="display:flex; :space-between;font-size:30px;">
                               
                                    <a onclick= "return confirm(\'voulez vous vraiment desarchiver?\')" href="desarchiver.php?matricule=' . $data['matricule'] . '">
